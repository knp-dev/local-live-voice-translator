...
@dataclass
class CustomKokoroTTSOptions(TTSOptions):
    voice: str = "af_heart"
    speed: float = 1.0
    lang: str = "en-us"
    is_phonemes: bool = False

class CustomKokoroTTSModel(TTSModel):
    def __init__(self):
    ...
    def tts(
        self, text: str, options: CustomKokoroTTSOptions | None = None
    ) -> tuple[int, NDArray[np.float32]]:
        options = options or CustomKokoroTTSOptions()
    ...
    async def stream_tts(
        ...
        for s_idx, sentence in enumerate(sentences):
            ...
            async for chunk in self.model.create_stream(
                sentence, voice=options.voice, speed=options.speed, lang=options.lang, is_phonemes=options.is_phonemes
            ):
